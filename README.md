...
簡單、diff 一路累積。中途要不要 commit 由你用 git 節點明確決定；整個 run 跑完，
剩下的變更會自動 commit 到 `task/<run-id>` 上 —— 成果只留在 worktree 裡的話，
branch 是空的、UI 給的合併指令是假的，worktree 一清就什麼都不剩。
...
按「執行」時如果有還沒存的變更，會先幫你存檔再跑（覆蓋目前開著的那個工作流）。
run 自己也會存一份圖的快照，但那份在執行紀錄裡，不是專案裡的工作流檔案。
